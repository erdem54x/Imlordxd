...
module.exports = {
    ...
    Kullanim: "y u <@passenger/ID> <Yetki S.Kodu>",
  ...
  onRequest: async function (client, message, args) {
    ...
    if(sistem.botSettings.Tag.some(tag => !uye.user.tag.toLowerCase().includes(tag))) return message.react(message.guild.emojiGöster(emojiler.Iptal) ? message.guild.emojiGöster(emojiler.Iptal).id : undefined)
        ...
        rol.members.forEach(async uye => {
          if (uye.user.bot) return;
            if (_statSystem.staffs.some(x => uye.roles.cache.has(x.rol))) {
              let passenger = _statSystem.staffs.find(x => uye.roles.cache.has(x.rol))
	      let No = Number(passenger.No)
          if(!uye.roles.cache.has(roller.altilkyetki)) await uye.roles.add(roller.altilkyetki);
