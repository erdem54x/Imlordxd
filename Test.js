  ...
  onLoad: function (client) {
    client.on('interactionCreate', async i => {
      ...
      let RowTwo = new MessageActionRow().addComponents(
        ...
        if (i.customId === 'passengeryöneticipaneli') {
          ...
          if(i.values[0] == "tagsızver") {
            ...
            let tagsizuye = guild.members.cache.filter(m => sistem.botSettings.Tag.some(tag => m.user.tag.toLowerCase().includes(tag)) && !m.roles.cache.has(roller.tagRolü) && !m.roles.cache.has(roller.şüpheliRolü) && !m.roles.cache.has(roller.yasaklıTagRolü) &&  !m.roles.cache.has(roller.jailRolü) && !roller.kayıtsızRolleri.some(x => m.roles.cache.has(x)))
            tagsizuye.forEach(roluolmayanlar => { 
              roluolmayanlar.roles.add(roller.tagRolü).catch(err => {})
              roluolmayanlar.setNickname(roluolmayanlar.displayName.replace(ayarlar.tagsiz)).catch(err => {})
            });
            let rollütagsiz = guild.members.cache.filter(m => m.roles.cache.has(roller.tagRolü) && sistem.botSettings.Tag.some(tag => !m.user.tag.toLowerCase().includes(tag)) && !m.roles.cache.has(roller.şüpheliRolü) && !m.roles.cache.has(roller.yasaklıTagRolü) && !m.roles.cache.has(roller.jailRolü) && !roller.kayıtsızRolleri.some(x => m.roles.cache.has(x)))
            rollütagsiz.forEach(rl => {
                rl.setNickname(rl.displayName.replace(ayarlar.tagsiz)).catch(err => {})
...
`, ephemeral: true})
  ...
  .addComponents(
    ...
    .setMinLength(3)
    .setMaxLength(60)
    .setPlaceholder(`Örn: Passenger'ın Odası`)
    .setRequired(true),
    new TextInputComponent()
