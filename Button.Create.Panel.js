...
${uye} isimli üyenin <t:${String(Date.now()).slice(0,10)}:F> tarihinde aşağıda bot sorunu bildirdi.`)
        ...
        await modal.deferReply({ ephemeral: true })
        await modal.followUp({content: `Başarıyla bot sorunu passenger'a iletilmiştir. Teşekkür Ederiz! ${guild.emojiGöster(emojiler.Onay)}` , ephemeral: true })
      }
      ...
      if(i.customId == "sorunÇözmeci") {
       ...
       if(i.customId == "basvurucuk") {
          ...
          .addComponents(
            ...
            .setMinLength(5)
            .setMaxLength(25)
            .setPlaceholder('Örn: Passenger 20')
            .setRequired(true),
            new TextInputComponent()
            ...
            .setMinLength(5)
            .setMaxLength(100)
            .setPlaceholder('Örn: passenger#0001/ID')
            .setRequired(false),
            new TextInputComponent()
...
\` 2. Adım \` Kayıt yapmak için \`.kayıt <@passenger/ID> <İsim> <Yaş>\` komutu ile kayıt işlemine başlarsın orda konuştuğun üyenin cinsiyetini belirleyerek kayıt işlemini gerçekleştirirsin ve **5 Saniye** sonra Sohbet odalarına otomatik olarak üyeyi taşıma işlemi yapar.
...
**Puan Detay Bilgisi Görseli**`, image: "https://cdn.discordapp.com/attachments/919637452648493116/942310714989772841/unknown.png" ,category: "kaçpuan"}
            ...
            if(ayarlar.type) info.push({header: "Taglı Nasıl Kullanılır?", description: `\` TAGLI (Tagged) \` Sunucumuz da davet ettiğin, kayıt ettiğin ve arkadaşlarını taga davet edebilirsin ettiğin arkadaşların ile puan kazanmak istemez misin? Ozaman \`${global.sistem.botSettings.Prefixs[0]}tag <@passenger/ID>\` komutu ile taglı daveti gönderebilirsin ama ondan önce kesinlikle isminin ister başına ister sonuna \` ${sistem.botSettings.Tag} \` sembolünü koymalıdır ayrıca taga davet ettiğin üyeleri \`${global.sistem.botSettings.Prefixs[0]}taglılarım\` komutu ile görüntüleyebilirsin.
...
await message.react(message.guild.emojiGöster(emojiler.Onay) ? message.guild.emojiGöster(emojiler.Onay).id : undefined).catch(err => {})
                  ...
                  if(i.customId == "yöneticiPaneli") {
                    ...
                    let Row = new MessageActionRow().addComponents(
                      new MessageSelectMenu()
                      .setCustomId("passengeryöneticipaneli")
