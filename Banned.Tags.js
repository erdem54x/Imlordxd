...
\`\`\``).catch(err => {})
        ...
        if (["kaldır", "sil", "remove", "delete", "Sil","çıkar","Çıkart"].some(kontrol => kontrol === args[0])) {
            ...
            const findUser = ayar.yasakTaglar.find(passenger => passenger == tags);
...
Üzerinizde bulunan **\` ${args[1]} \`** bu sembol veya etiket yasağı kaldırıldığından dolayı sizi yasaklı kategorisinden çıkarttık.`).catch(err => {})
                    ...
                    if(!ayarlar.taglıalım && User && User.Name && User.Names && User.Gender) {
                        if(uye && uye.manageable && ayarlar.type && ayarlar.isimyas) uye.setNickname(`${ayarlar.tagsiz} ${User.Name}`)
                        ...
                        if(sistem.botSettings.Tag.some(tag => uye.user.tag.toLowerCase().includes(tag))) uye.roles.add(roller.tagRolü).catch(err => {})
                    } else {
                        uye.setRoles(roller.kayıtsızRolleri).catch(err => {})
                        if(uye && uye.manageable && ayarlar.type && ayarlar.isimyas) await uye.setNickname(`${ayarlar.tagsiz} İsim | Yaş`)
                        ...
                        if(uye && uye.manageable && ayarlar.type && !ayarlar.isimyas) await uye.setNickname(`${ayarlar.tagsiz} Kayıtsız`)
                    }
                }, 1500)
            });
            };
